...
def _load_indices_cache():
    indices_file = os.path.join(
        os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
        "service", "models", "indices_cache.json",
    )
    if os.path.exists(indices_file):
        try:
            ...
            print(f"[WARN] Failed to read indices_cache.json: {e}")
    return None


def fetch_market_indices(timeout_sec: float = 1.8):
    """Fetch live/cached Nifty 50 and BSE 100 / Sensex index prices with strict timeout."""
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            future = executor.submit(fetch_index_quotes)
            _INDEX_CACHE = future.result(timeout=timeout_sec)
            _INDEX_CACHE_TIME = now
            return _INDEX_CACHE
    except Exception as e:
        print(f"[WARN] Live index fetch failed or timed out ({timeout_sec}s): {e}")

    # EOD cache fallback: check indices_cache.json generated by daily workflow
    eod_cache = _load_indices_cache()
    if eod_cache and eod_cache.get("nifty50") and eod_cache.get("bse100"):
        _INDEX_CACHE = eod_cache
        _INDEX_CACHE_TIME = now
        return _INDEX_CACHE
...
def scan_tickers(
    ...
    if cache_total == 0:
        cache_status = "MISSING_OR_EMPTY"
    elif len(results) == 0:
        cache_status = "FILTER_NO_MATCH"
    else:
        cache_status = "LOADED"
    ...
    return {
        ...
        "cache_ticker_count": cache_total,
        "indices": indices,
        "total": len(results),
